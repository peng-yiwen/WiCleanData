...
def load_clean_taxonomy(path: str):
    ...
    with open(path) as f:
        for line in f:
            ...
            if len(parts) != 2:
                continue
            child, parent = parts[0].strip()[3:], parts[1].strip()[3:]
...
def load_relation_constraint_types(
    ...
    with open(subject_csv) as f:
        ...
        for row in reader:
            if _normalize_property_id(row.get("property", "")) != prop:
                continue
            subjects.append((row["subject_type"], row["subject_type_label"]))
    ...
    with open(value_csv) as f:
        ...
        for row in reader:
            if _normalize_property_id(row.get("property", "")) != prop:
                continue
            objects.append((row["value_type"], row["value_type_label"]))
...
def main():
    ...
    parser.add_argument(
        "--relation",
        default="P1000",
        help="Wikidata property id (e.g. P1000); subject/object types are read from constraint CSVs.",
    )
    parser.add_argument(
        "--subject-constraints",
        default=None,
        help="rel_subject_type_constraints.csv path (default: same directory as chunk.py)",
    )
    parser.add_argument(
        "--value-constraints",
        default=None,
        help="rel_value_type_constraints.csv path (default: same directory as chunk.py)",
    )
    parser.add_argument("--taxonomy", default="wikc_plus.txt")
    parser.add_argument(
        "--types",
        default=None,
        help="Deprecated: single subject or value constraints CSV. If set, runs legacy single-file mode (ignores --relation).",
    )
