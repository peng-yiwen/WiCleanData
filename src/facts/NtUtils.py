    ...
    def mainSubject(self):
        ...
        for key in self.index:
            if not key.startswith("s:"):
                self.mainSubjectCache=key # the main subject
                return key
        return None
    def __len__(self):
        return len(self.index)
    def numberOfTriples(self):
        return sum(len(self.index[s][p]) for s in self.index for p in self.index[s])
...
def about(triple):
    ...
    s,p,o=triple
    # No schema:about in Wikidata truthy version
    # if p=="schema:about":
    #     s=o

    # Remove redundant labels and strict to english version 
    p_list = {
        Prefixes.rdfsLabel, 
        Prefixes.schemaName, 
    ...
    if p in p_list and not o.endswith("@en"): # strict to english version!!
        return None
    
    # Discard meta statements
...
def visitWikidataEntities(args):
    ...
    with open(file,"rb", buffering=1*mega) as wikidataReader:
        ...
        line=b"NONE"
        for line in wikidataReader:
            if line.rstrip().endswith(b"<http://wikiba.se/ontology#Item> .") or line.rstrip().endswith(b"<http://schema.org/Dataset> ."):
...
if __name__ == '__main__':
    with open("test.nt", "tw", encoding="UTF-8") as f:
        for triple in triplesFromNtFile("../sample.nt"):
